...
These methods are implemented in `src/pymt5/mt5_service.py`, which wraps `package/helpers/mt5_account.py` low-level API with dataclass conversion for trading results.
...
1. **Complex dataclass conversion** (3 methods with HIGH/VERY HIGH value):
   ...
   - `check_order()`: Extracts deeply nested `mrpc_mql_trade_check_result` → OrderCheckResult (8 fields)
...
2. **Simple unpacking** (3 methods with LOW value):
   ...
   - `close_order()`: Unpacks `data.returned_code` → int
   - `calculate_margin()`: Unpacks `data.margin` → float
   - `calculate_profit()`: Unpacks `data.profit` → float
